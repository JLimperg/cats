...
open import Cats.Category.Discrete Bool as Two using () renaming (Discrete to Two)
...
module _ {lo la l≈} {Cat : Category lo la l≈} {A B : Category.Obj Cat} where
  ...
  open Cone using (Apex ; arr)
  open Limit using (cone)
  ...
  cone→productData c = Apex c , arr c true , arr c false
  ...
  product {c} term {X} xl xr = record
    ...
    where
      ...
      prop = ≈.sym (commute (u ↓) _) , ≈.sym (commute (u ↓) _)
  ...
  limit→product L
      = let (P , pl , pr) = cone→productData (L ↓) in
        record
          { prod = P
          ; projl = pl
          ; projr = pr
  ...
  product-unique : (P Q : BinaryProduct A B) → P ↓ ≅ Q ↓
